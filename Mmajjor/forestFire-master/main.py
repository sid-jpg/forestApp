...
def is_water_body(location_name):
    """Check if the location is a water body"""
    water_body_keywords = ['ocean', 'sea', 'lake', 'river', 'bay', 'gulf']
    for keyword in water_body_keywords:
        if keyword in location_name.lower():
            return True
    return False

def is_non_forest_area(location_name):
    """Check if the location is a non-forest area"""
    non_forest_area_keywords = ['city', 'town', 'village', 'urban', 'desert', 'grassland']
    for keyword in non_forest_area_keywords:
        if keyword in location_name.lower():
            return True
    return False
