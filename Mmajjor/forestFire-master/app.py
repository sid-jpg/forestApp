...
def is_water_body(location):
    water_bodies = ['sea', 'ocean', 'lake', 'river']
    return any(water in location.lower() for water in water_bodies)

def is_non_forest_area(location):
    non_forest_areas = ['city', 'desert', 'building', 'landscape']
    return any(area in location.lower() for area in non_forest_areas)
...
def get_location_name(lat, lon):
    try:
        ...
        data = response.json()
        return data['address']['city'] if 'city' in data['address'] else data['address']['town'] if 'town' in data['address'] else data['address']['village'] if 'village' in data['address'] else 'Unknown'
...
def risk():
    ...
    # Check if the location is a water body
    if is_water_body(location_name):
        return jsonify({'message': 'Water body detected at this location.'})

    # Check if the location is a non-forest area
    if is_non_forest_area(location_name):
        return jsonify({'message': 'This location is not a forest area.'})

    # Include the city name in the risk panel
    return jsonify({'city': location_name, 'risk': 'Risk assessment in progress...'})

@app.route('/admin', methods=['GET', 'POST'])
def admin_panel():
    if request.method == 'POST':
        # Logic to manage models and send alerts
